...
$statusFiltered = (string) $filter->filter($status)->getFilteredText();
if (mb_strlen($statusFiltered, 'UTF-8') > 280) {
...
$insert_stmt->bindParam(':status', $statusFiltered, PDO::PARAM_STR);
$insert_stmt->bindParam(':created_at', $time_posted, PDO::PARAM_STR);
if ($insert_stmt->execute()) { 
    exit('{"success": true, "message": "Status updated successfully."}');
} else {
    exit('{"success": false, "message": "Could not update status. Please try again later."}'); // trapz was here cuhhh
}
