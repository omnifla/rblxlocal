...
use Roblox\RobloxGuestCookie;
// this is the join script for the Roblox Game Client.
header("Content-Type: text/plain");

$user = Auth::GetAuthenticatedUserInfo();
if (!$user) {
    // if the user is not authenticated, we will use a guest cookie
    $guestCookie = RobloxGuestCookie::GetOrCreate(rand(-2147483647, -1));
    $user = [
...
ob_clean(); // ALWAYS PUT THIS AT THE BOTTOM OF THE SCRIPT TO PREVENT ANY OUTPUT BEFORE THE HEADERS ARE SENT.
?>
-- functions --------------------------
...
pcall(function()
	...
	game:HttpGet("<?= $baseUrl ?>Game/JoinRate.ashx?st=0&i=0&p=-1&c=GameAppInit&r=Success&d=" .. (math.floor(t*1000)) .. "&ip=localhost&errorType=&platform=" .. platform, false)
end)
...
function reportDuration(category, result, duration, blocking,errorType)
	...
	pcall(function() game:HttpGet("<?= $baseUrl ?>Game/JoinRate.ashx?st=0&i=0&p=-1&c=" .. category .. "&r=" .. result .. "&d=" .. (math.floor(duration*1000)) .. "&b=" .. bytesReceived .. "&ip=localhost&errorType=" .. errorType .. "&platform=" .. platform, blocking and not isTouchDevice) end)
end
-- arguments ---------------------------------------
...
pcall(function() game:SetVideoInfo('<?= $video_info ?>') end)
